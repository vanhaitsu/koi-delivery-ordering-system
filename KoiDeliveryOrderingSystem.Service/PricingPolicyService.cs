    ...
    public class PricingPolicyService
    {
        ...
        public async Task<IBusinessResult> Save(PricingPolicy pricingPolicy)
        {
            try
            {
                ...
                PricingPolicy pricingPolicyTmp = await _unitOfWork.PricingPolicyRepository.GetByIdAsync(pricingPolicy.PricingId);

                if (pricingPolicyTmp != null)
                {
                    pricingPolicyTmp.WeightRange = pricingPolicy.WeightRange;
                    pricingPolicyTmp.ShippingMethod = pricingPolicy.ShippingMethod;
                    pricingPolicyTmp.BasePrice = pricingPolicy.BasePrice;
                    pricingPolicyTmp.AdditionalServicePrice = pricingPolicy.AdditionalServicePrice;
                    pricingPolicyTmp.InsuranceFee = pricingPolicy.InsuranceFee;
                    pricingPolicyTmp.CustomsFee = pricingPolicy.CustomsFee;
                    pricingPolicyTmp.Currency = pricingPolicy.Currency;
                    pricingPolicyTmp.EffectiveDate = pricingPolicy.EffectiveDate;
                    pricingPolicyTmp.ExpiryDate = pricingPolicy.ExpiryDate;

                    result = await _unitOfWork.PricingPolicyRepository.UpdateAsync(pricingPolicyTmp);

                    if (result > 0)
                    {
                        return new BusinessResult(Const.SUCCESS_UPDATE_CODE, Const.SUCCESS_UPDATE_MSG, pricingPolicyTmp);
                    }
                    else
                    {
                        return new BusinessResult(Const.FAIL_UPDATE_CODE, Const.FAIL_UPDATE_MSG);
